...
export function EmployerAction({ icon, label, hint, onPress, disabled = false, tone = 'secondary', busy = false }: { icon: IconName; label: string; hint?: string; onPress?: () => void; disabled?: boolean; tone?: 'primary' | 'secondary' | 'quiet' | 'danger'; busy?: boolean }) {
  ...
  return (
    <TouchableOpacity
      style={[styles.action, tone === 'primary' && styles.actionPrimary, tone === 'quiet' && styles.actionQuiet, tone === 'danger' && styles.actionDanger, unavailable && styles.disabled]}
      onPress={onPress}
...
const styles = StyleSheet.create({
  banner: { flexDirection: 'row', gap: 12, borderRadius: 18, padding: 14, backgroundColor: tokens.colors.brandDark, overflow: 'hidden', ...tokens.shadow.card },
  ...
  accordionHeader: { minHeight: 62, flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 13, paddingVertical: 10 }, accordionIcon: { width: 36, height: 36, borderRadius: 11, alignItems: 'center', justifyContent: 'center', backgroundColor: tokens.colors.brandSoft },
