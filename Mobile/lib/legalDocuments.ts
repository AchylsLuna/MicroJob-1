/**
 * Mirrors `client/src/constants/legalDocuments.ts` verbatim (same section
 * copy, same document ids). `client/src` and `Mobile/` are separate npm
 * workspaces with separate bundlers, so this is duplicated rather than
 * shared — same reasoning as `Mobile/theme/categoryVisuals.ts`. Keep both
 * files in sync by hand; do not rewrite the legal copy from scratch here.
 */
