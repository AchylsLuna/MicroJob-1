...
function AdminDashboardContent() {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) => {
      ...
      const locationText = (job as any).location ? String((job as any).location).toLowerCase() : "";
      const matchesSearch =
        !normalizedSearch ||
  ...
  return (
    <div className="max-w-full ml-6 md:ml-12 space-y-6 text-[16px] mt-8 md:mt-12">
      <header className="mb-8 text-left">
        <h1 className="text-4xl md:text-5xl font-semibold text-[#111827]">Admin Dashboard</h1>
        <p className="text-base text-[#6B7280] mt-2">Overview of your job posting platform</p>
      </header>
      {loadError && (
