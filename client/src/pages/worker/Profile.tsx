...
const formatAcceptedDate = (value?: string) => {
...
export function Profile() {
  ...
  useEffect(() => {
    ...
    const loadAcceptedWorks = async () => {
      try {
        ...
        const mapped = (Array.isArray(applications) ? applications : [])
          ...
          .map((application: any) => {
            ...
            return {
              ...
              company,
              companyLogo,
              completedDate: formatAcceptedDate(application.updatedAt || job.updatedAt),
        ...
        if (isMounted) {
          setAcceptedWorks([]);
          setAcceptedWorksError(error instanceof Error ? error.message : "Failed to load accepted work.");
        }
      } finally {
  ...
  const location = profileUser?.city || "Location not set";
  ...
  const profileData = {
    ...
    phone: profileUser?.phoneNumber || "Not set",
    location,
    linkedin: profileUser?.linkedin || "",
    website: profileUser?.website || "",
    about: profileUser?.about || "No profile summary added yet.",
  };
    ...
    <div className="max-w-[1100px] mx-auto space-y-6">
      ...
      <div className="rounded-[16px] border border-[#BFDBFE] bg-[#EFF6FF] p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-[15px] font-semibold text-[#1E3A8A]">Profile completeness</p>
            <p className="mt-1 text-[13px] text-[#475569]">
              {profileCompletion === 100 ? "Your profile is ready for employers." : "Add the missing details to help employers evaluate you faster."}
            </p>
          </div>
          <div className="flex items-center gap-3">
            {profileLoading ? <RefreshCw className="h-4 w-4 animate-spin text-[#1C4D8D]" aria-label="Refreshing profile" /> : null}
            <span className="text-[22px] font-bold text-[#1C4D8D]">{profileCompletion}%</span>
          </div>
        </div>
        <div className="mt-4 h-2 overflow-hidden rounded-full bg-white" role="progressbar" aria-label="Profile completeness" aria-valuemin={0} aria-valuemax={100} aria-valuenow={profileCompletion}>
        ...
        <div className="p-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="flex items-center gap-3 p-4 bg-[#f8fafc] rounded-[12px] border border-[#e2e8f0]">
                        ...
                        <div>
                          ...
                          {profileUser?.email ? <a href={`mailto:${profileUser.email}`} className="text-[14px] font-medium text-[#1e293b] hover:text-[#1C4D8D] break-all">{profileData.email}</a> : <p className="text-[14px] text-[#64748B]">Not set</p>}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 p-4 bg-[#f8fafc] rounded-[12px] border border-[#e2e8f0]">
                        ...
                        <div>
                          ...
                          {profileUser?.phoneNumber ? <a href={`tel:${profileUser.phoneNumber}`} className="text-[14px] font-medium text-[#1e293b] hover:text-[#1C4D8D]">{profileData.phone}</a> : <p className="text-[14px] text-[#64748B]">Not set</p>}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 p-4 bg-[#f8fafc] rounded-[12px] border border-[#e2e8f0]">
                        ...
                        <div>
                          ...
                          {safeLinkedinUrl ? <a href={safeLinkedinUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[14px] font-medium text-[#1e293b] hover:text-[#0A66C2]">Open profile <ExternalLink className="h-3.5 w-3.5" /></a> : <p className="text-[14px] text-[#64748B]">Not set</p>}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 p-4 bg-[#f8fafc] rounded-[12px] border border-[#e2e8f0]">
                        ...
                        <div>
                          ...
                          {safeWebsiteUrl ? <a href={safeWebsiteUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[14px] font-medium text-[#1e293b] hover:text-[#9333EA]">Visit website <ExternalLink className="h-3.5 w-3.5" /></a> : <p className="text-[14px] text-[#64748B]">Not set</p>}
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {activeTab === "skills" && (
            <div className="space-y-6">
              ...
              <div>
                ...
                {workExperiences.length ? (
                    ...
                    {workExperiences.map((item, index) => (
                      <div key={item._id || item.id || `${item.title}-${index}`} className="flex gap-4 rounded-[14px] border border-[#E2E8F0] bg-[#F8FAFC] p-5">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-[12px] text-[#64748B]">{formatExperienceDate(item.startDate)} – {item.current ? "Present" : formatExperienceDate(item.endDate)}</p>
                  ...
                  <div className="rounded-[16px] border border-dashed border-[#CBD5E1] bg-[#F8FAFC] py-8 text-center">
                    ...
                    <p className="text-[13px] text-[#64748B]">No work history added yet.</p>
                  </div>
                )}
              </div>
              ...
              {/* Skills List */}
              <div className="flex items-center justify-between">
                <h3 className="text-[16px] font-semibold text-[#1E293B]">Skills</h3>
                <span className="text-[12px] text-[#64748B]">{skills.length} {skills.length === 1 ? "skill" : "skills"}</span>
              </div>
              {skills.length > 0 ? (
