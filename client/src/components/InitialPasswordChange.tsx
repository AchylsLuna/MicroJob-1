...
export function InitialPasswordChange() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (newPassword !== confirmPassword) return setError(t("initialPasswordChange.errors.mismatch"));
    setIsSubmitting(true);
    try {
      await changeInitialPassword({ currentPassword, newPassword });
      updateProfile({ passwordChangeRequired: false });
      toast.success(t("initialPasswordChange.toast.success"));
      navigate(getPostAuthLandingPath(user), { replace: true });
    } catch (requestError: any) {
      setError(requestError?.message || t("initialPasswordChange.errors.changeFailed"));
    } finally {
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-10 flex items-center justify-center">
      <form onSubmit={handleSubmit} className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" noValidate>
        ...
        <button type="submit" disabled={isSubmitting} className="brand-primary-interactive mt-6 w-full rounded-xl px-4 py-3 font-semibold">
          {isSubmitting ? t("initialPasswordChange.form.submitLoading") : t("initialPasswordChange.form.submit")}
        </button>
      </form>
    </main>
  );
