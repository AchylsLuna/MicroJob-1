...
// Config-object factories (not module-level constants) so tab labels stay in
// sync with the active language — call via useMemo(() => getX(t), [t]).
...
// The predefined skill picker's <option value> is the canonical (English)
// skill name persisted to the backend; only the displayed label is
// translated via settings.skills.catalog.items.<key> / .categories.<key>.
...
export function Settings() {
  ...
  const handleUploadIdentityDocument = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(t("settings.toast.identityUploaded"));
      // Reload verification status
      await reloadVerificationStatus();
    } catch (error: any) {
      toast.error(error?.message || t("settings.toast.identityUploadFailed"));
    }
  };
  ...
  return (
    <div className="ui-page mx-auto max-w-[1200px] px-4 pb-16 md:px-0">
                      ...
                      {!isAdminRole && (
                        <>
                          ...
                          <datalist id="settings-address-options">
                            {addressSuggestionKeys.map((key) => (
                              <option key={key} value={t(`settings.personal.addressSuggestions.${key}`)} />
                            ))}
                          </datalist>
                        </>
                      )}
                            ...
                            {resolvedAvatarUrl ? (
                              <div className="flex items-center gap-4">
                                <img
                                  ...
                                  alt={t("settings.personal.personalHeading")}
                              ...
                              <div className="flex flex-wrap items-center gap-4">
                                <label className="bg-[#1C4D8D] text-white font-semibold px-6 py-3 rounded-[10px] hover:opacity-90 transition-all cursor-pointer flex items-center gap-2 focus-within:ring-2 focus-within:ring-[#1C4D8D] focus-within:ring-offset-2">
                                  ...
                                  <input
                                    ...
                                    onChange={handlePhotoUpload}
                                    disabled={isAvatarSubmitting}
                                    aria-label={t("settings.personal.uploadPhoto")}
                                    className="sr-only"
                                  />
                                </label>
                            ...
                            {skillSelectionMode === "predefined" ? (
                              ...
                              >
                                ...
                                {SKILL_CATALOG.map((category) => (
                                  <optgroup key={category.categoryKey} label={t(`settings.skills.catalog.categories.${category.categoryKey}`)}>
                              ...
                              <div className="flex-1">
                                ...
                                {step.id === "identity" && (step.status === "pending" || step.status === "rejected") && (
                                  <label className="mt-2 inline-block px-4 py-2 bg-[#1C4D8D] text-white text-[12px] rounded-[8px] hover:opacity-90 cursor-pointer">
                                    {step.status === "rejected" ? t("settings.verification.uploadReplacementId") : t("settings.verification.uploadId")}
                                    <input
                                      type="file"
                                      accept="image/*,.pdf"
      ...
      <ConfirmDialog
        ...
        description={t("settings.deleteExperienceDialog.description", { title: deleteExperienceTarget?.title || t("settings.deleteExperienceDialog.fallbackTitle") })}
