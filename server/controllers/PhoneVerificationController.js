import { isValidPhoneNumber, normalizePhoneNumber } from '../lib/phone/phoneUtils.js';
...
export async function sendPhoneCode(req, res) {
    try {
        ...
        if (!user || !user.phoneNumber) {
            return res.status(400).json({ error: 'Please set a phone number in your profile first' });
        }

        const raw = user.phoneNumber;
        const formatted = normalizePhoneNumber(raw);
        console.info('Sending phone code', { userId, raw, formatted });
        ...
        await PhoneVerification.create({ user: userId, pinCode: otp, expiresAt });
...
export async function resendPhoneCode(req, res) {
    try {
        ...
        const raw = user.phoneNumber;
        const formatted = normalizePhoneNumber(raw);
        console.info('Resending phone code', { userId, raw, formatted });
